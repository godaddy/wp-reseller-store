...
( function ( $ ) {
	...
	var cart = {
		...
		validateSecret: function ( secret ) {
			return $( 'iframe.wp-embedded-content' ).filter(
				function ( index, item ) {
			...
			$.ajax(
				{
					...
					success: function ( response ) {
						if ( response.status === 'partial' ) {
							$( '.rstore-welcome-block span.firstname' ).each(
								function () {
									$( this ).text( response.name );
								}
							);
							$( '.rstore-welcome-block span.lastname' ).each(
								function () {
									$( this ).text( response.lastname );
								}
							);
							$( '.rstore-welcome-block span.shopper-id' ).each(
								function () {
									$( this ).text( response.shopperid );
								}
							);

							$( '.rstore-welcome-block' ).each(
								function () {
									$( this ).show();
								}
							);

							$( '.rstore-login-block' ).each(
								function () {
									$( this ).hide();
								}
							);
	...
	var domain = {
		...
		init: function () {
			if ( $( '.rstore-domain-popup' ).length ) {
				...
				$( 'body' ).on(
					'click',
					'#rstore-closePop, #rstore-blackout',
					function () {
