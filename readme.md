<!-- DO NOT EDIT THIS FILE; it is auto-generated from readme.txt -->
# GoDaddy Reseller Store

![Banner](wp-org-assets/banner-1544x500.png)
Resell hosting, domains, and more right from your WordPress site.

**Contributors:** [godaddy](https://profiles.wordpress.org/godaddy)  
**Tags:** [admin](https://wordpress.org/plugins/tags/admin), [posts](https://wordpress.org/plugins/tags/posts), [users](https://wordpress.org/plugins/tags/users)  
**Requires at least:** 4.6  
**Tested up to:** 4.7  
**Stable tag:** 0.1.0  
**License:** [GPL-2.0](https://www.gnu.org/licenses/gpl-2.0.html)  

[![Build Status](https://travis-ci.org/godaddy/wp-reseller-store.svg?branch=master)](https://travis-ci.org/godaddy/wp-reseller-store) [![Built with Grunt](https://cdn.gruntjs.com/builtwith.svg)](http://gruntjs.com) [![devDependency Status](https://david-dm.org/godaddy/wp-reseller-store/dev-status.svg)](https://david-dm.org/godaddy/wp-reseller-store#info=devDependencies) 

## Description ##

TODO

## Changelog ##

### 0.1.0 - January 2017 ###
* Initial release

Props [@fjarrett](https://github.com/fjarrett), [@bfocht](https://github.com/bfocht)


